        ...
        <c:forEach var="part" items="${event.parts}">
            <tr>
                <td>
                    <c:choose>
                        <c:when test="${event.singleDay == true}">
                            <em><joda:format value="${part.startDate}" pattern="HH:mm"/> -
                                <joda:format value="${part.endDate}" pattern="HH:mm"/></em>
                        </c:when>
                        <c:otherwise>
                            <em><joda:format value="${part.startDate}"
                                             pattern="dd MMMM yyyy, HH:mm"/> -
                                <joda:format value="${part.endDate}"
                ...
                <li><em>${paper.title}</em>
                    <c:if test="${not empty paper.authors}">
                        ...
                        <c:forEach var="author" items="${paper.authors}" varStatus="count">
                            ${author.name}<c:choose><c:when test="${not count.last}">;</c:when><c:otherwise>.</c:otherwise></c:choose>
                        </c:forEach>
                    </c:if>
                    <c:if test="${paper.retrievable}">[ <a href="${paper.id}">Download</a> ]
                        <img class="externalLink" src="./images/web.png" alt="External Link" width="16" height="16"/></c:if>
                </li>
            </c:forEach>
        </ul>
    </div>
